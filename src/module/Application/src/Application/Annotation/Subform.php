...
use Application\Exception\Annotation\Subform\GetterDoesNotExist;
use Application\Exception\Annotation\Subform\GetterNotSet;
use Application\Exception\Annotation\Subform\InvalidSubEntity;
...
class Subform extends Field
{
    ...
    const DEFAULT_TEMPLATE = 'partials/form/children.phtml';
    ...
    public function populateFromParent(Form $annotatedForm, EntityToForm $e2f)
    {
        ...
        if (!$subentity instanceof \Application\Entity\BaseInterface) {
            throw new InvalidSubEntity("Sub entity must implement \\Application\\Entity\\BaseInterface");
        }
        $subform = new \Application\Annotation\Form($subFormElement, $e2f);
